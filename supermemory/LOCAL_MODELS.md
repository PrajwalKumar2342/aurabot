...
**Option B: Supermemory + Local Models** (Full memory system)
```bash
...
### Memory Operations (Supermemory server only)
...
|----------|---------|-------------|
| `SUPERMEMORY_HOST` | `localhost` | Server bind address |
| `SUPERMEMORY_PORT` | `8000` | Server port |
| `MODELS_DIR` | `./models` | Directory for downloaded models |
| `SUPERMEMORY_API_KEY` | (required) | Your Supermemory API key |

Example:
```bash
set SUPERMEMORY_PORT=8080
set SUPERMEMORY_API_KEY=your_api_key
python supermemory_local.py
```
...
### `supermemory_local.py`
Full Supermemory memory server with local models.
- Requires `supermemory` package
- Provides memory storage, search, and retrieval
- Uses Supermemory.ai cloud for vector storage
...
### Supermemory authentication error
- Ensure `SUPERMEMORY_API_KEY` is set correctly
- Get your API key from https://supermemory.ai -> Developer Platform
